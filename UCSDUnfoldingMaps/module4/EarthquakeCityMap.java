...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		triangle(45,120,55,130,50,130);//city marker
		
		fill(color(255,255,255));
		ellipse(50, 175, 15, 15);//land quake
		
		fill(color(250,255,255));
		rect(50, 218, 15, 15);//ocean quake
		
		fill(color(255, 255, 0));
		ellipse(50, 320, 10, 10);//shallow yellow
		
		fill(color(0, 0, 255));
		ellipse(50, 370, 10, 10);//int blue
		

		fill(color(255,0,0));
		ellipse(50, 420, 10, 10);//deep red
		
		fill(0, 0, 0);
		text("City Marker", 75, 125);
		text("Land Quake", 75, 175);
		text("Ocean Quake", 75, 225);
		text("Size-Magnitude", 60, 275);
		text("Shallow", 75, 325);
	...
	private void printQuakes() 
	{
		...
		//   Loop over all of the countries, e.g. using 
		 HashMap<String,Integer> cno= new HashMap<String,Integer>();
		 int ocqk=0;
		 int cntry=0;
		 ...
		 {	
			 ...
			 for( Marker em : quakeMarkers ) 
			 {
				 ...
				 if(name== em.getProperty("country"))
					 ctr++;
				 }
				 else ocqk++;
			 ...
			 int ccc=0;
			 if(cno.containsKey(name))
			      ccc=cno.get(name);
			 cno.put(name,ctr+ccc);
			 
		 } 
		 System.out.println(ocqk/cntry);
		 for (String aa:cno.keySet()) 
		 {	System.out.println(aa+":"+cno.get(aa));}
