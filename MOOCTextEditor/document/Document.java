...
public abstract class Document {
	...
	/*protected int countSyllables(String word)
	{
		...
		String vow="eaiouyAEIOUY";
		int num=0;
		for(int i=0;i<word.length();i++)
		{
			char a = word.charAt(i);
			int b= vow.indexOf(a);//if word is vowel
			if (b!=-1)
			{
				num++;
				for(int zz=i;zz<word.length();zz++)
				{
					if(word.charAt(zz) == a) continue;
					else {i=zz; break;}
				}
			}
			//System.out.print(b);
		...
		if (word.charAt(word.length()-1)=='e' && word.charAt(word.length()-2)!='e' && vow.indexOf(word.charAt(word.length()-2))==-1 )
	    num--;
	...
	*/
	...
	public double getFleschScore()
	{
		...
		double ans = 0.0;
		double p1 = 1.015 * getNumWords() * 1.0 / getNumSentences();
		double p2 = 84.6 * getNumSyllables() * 1.0 / getNumWords();
		ans = 206.835 - p1-p2;
	    return ans;
	}
